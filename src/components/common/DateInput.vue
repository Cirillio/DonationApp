...
const handleDayInput = (e: Event) => {
  ...
  if (value.length === 2) {
    const monthInput = input.parentElement?.querySelector(
      "[data-month]"
    ) as HTMLInputElement;
    monthInput?.focus();
  }
...
const handleMonthInput = (e: Event) => {
  ...
  if (value.length === 2) {
    const yearInput = input.parentElement?.querySelector(
      "[data-year]"
    ) as HTMLInputElement;
    yearInput?.focus();
  }
...
<template>
  <div
    class="grid h-fit w-full grid-cols-[auto_1fr_auto_1fr_auto_2fr] items-center border-b-2 border-primary/50 focus-within:border-primary px-3 py-2 text-base transition-all duration-150 outline-none [&[aria-invalid=true]]:border-destructive [&[aria-invalid=true]]:ring-destructive [&[aria-invalid=true]]:hover:border-destructive dark:[&[aria-invalid=true]]:ring-destructive/40"
  >
    <input
      :value="day"

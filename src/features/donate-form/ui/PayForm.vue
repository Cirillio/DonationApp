...
<template>
  <div class="flex flex-col gap-4">
    ...
    >
      ...
      <div class="relative">
        <FormControl>
          <Input ref="currencyRef" v-model="currencyFormatted" :placeholder="'0,00'" name="amount"
            @blur="handleBlur" @input="(e: HTMLInputElement) => handleNumericInput(e?.value, () => handleInput(e))"
            @change="() => setValue(currencyNumber)" inputmode="numeric" class="max-md:min-h-11 max-md:text-lg pr-10 " />
        </FormControl>
      </div>
      ...
      <div class="flex flex-wrap gap-1 w-full">
        <Button class="px-2 py-1 max-md:text-base" v-for="amount in PAYMENT_AMOUNTS" :key="amount.label"
      ...
      <FormItem class="gap-1">
        ...
        <FormControl>
          <div class="flex flex-col gap-2">
            <CheckBlock v-for="p in PAYMENT_TYPES" :key="p.type" class="w-full px-3 flex-1 max-md:min-h-14"
              @onCheck="(check: boolean) => check ? setValue(p.type) : setValue(undefined)" :showCheckbox="false"
